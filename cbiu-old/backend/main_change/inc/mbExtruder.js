	...
	$.mbExtruder= {
		...
		buildMbExtruder: function(options){
			return this.each (function (){
				...
				var extW;
				var c= $("<div/>").addClass("extruder-content ex");
				...
				{position:position,left:"50%",marginLeft:-this.options.width/2}:
								this.options.position=="bottom" ?
				{position:position,bottom:0,left:"50%",marginLeft:-this.options.width/2}:
				...
				if (isVertical){
					...
					var clicDiv=$("<div/>").css({position:"absolute",left:0,height:"100%",background:"#F7F7F7"});
					flap.append(clicDiv);
				}
		...
		adjustSize:function(){
			...
			if(isHorizontal){
				...
				if( $(window).width() < extruder.options.width){
					//$extruder.css({marginLeft:0, left:0});
//$extruder.find(".ext_wrapper, .extruder-content, .extruder-container").css({});
				}else{
					//$extruder.css({left:"50%",marginLeft:-extruder.options.width/2});
					//$extruder.find(".ext_wrapper, .extruder-content, .extruder-container").css({});
				}
				...
				if($extruder.attr("isOpened")){
					//$extruder.find('.extruder-content').css({});
					//$extruder.find(".extruder-content, .extruder-container").css({});
				}
			}
		...
		openMbExtruder:function(c){
			...
			//	extruder.find('.ext_wrapper').css({width:""});
				...
				extruder.find('.extruder-content').animate({}, opt.slideTimer,function(){
					extruder.find(".extruder-container").css({});
				});
				if(opt.onExtOpen) opt.onExtOpen();
			}
